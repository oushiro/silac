...
silac <- function(data = data()){
  ...
  apagar<-gsub("(a|ã|á|â|e|ê|é|i|í|o|õ|ó|ô|u|ú)(n)-(s)(b|c|ç|d|f|g|h|j|k|l|m|p|q|t|v|x|z|\\W)", "\\1\\2\\3-\\4", apagar, ignore.case=T)
...
#  apagar<-gsub("n-s-(m|b|c|f|g|l|p|t|v)", "ns-\\1", apagar)# <------- novo
  ...
  apagar<-gsub("-(b|d|f|g|k|p|t)\\b", "-\\1I", apagar)
  ...
  apagar<-gsub("a-o(s)?\b", "ao\\1", apagar)
  ...
  monossilabos.atonos<-c("o", "a", "e", "em", "n[ao]s?", "um", "[ao]s", "d[eao]s?", "pr[ao]s?", "que", "ou", "uns", "nuns", "[stm]e", "lhe", "mas", "com", "por", "[aoe]h", "num", "pra", "aos?") #... continuar lista ######!!!!!##
  ...
  ###PARTE 3 #https://raw.githubusercontent.com/oushiro/silac/master/
